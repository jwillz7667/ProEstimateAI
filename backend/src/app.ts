...
export function createApp() {
  ...
  v1.get("/generations/:id/preview", async (req, res, next) => {
    try {
      const { getPublicImageData } = await import(
        "./modules/generations/generations.service"
      );
  ...
  v1.get("/assets/:id/image", async (req, res, next) => {
    try {
      const { getPublicAssetImage } = await import(
        "./modules/assets/assets.service"
      );
  ...
  v1.get("/companies/:id/logo", async (req, res, next) => {
    try {
      const { getPublicCompanyLogo } = await import(
        "./modules/companies/companies.service"
      );
      ...
      if (!imageResult) {
        res.status(404).json({
          ...
          error: {
            code: "NOT_FOUND",
            message: "No logo set for this company",
          },
        });
        return;
      }
  ...
  // Property maps: address geocoding, lawn polygon area, roof scouting via
