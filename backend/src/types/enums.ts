...
// Commerce
export type PlanCode = 'FREE_STARTER' | 'PRO_MONTHLY' | 'PRO_ANNUAL';

export type EntitlementStatus =
  | 'FREE' | 'TRIAL_ACTIVE' | 'PRO_ACTIVE' | 'GRACE_PERIOD'
  | 'BILLING_RETRY' | 'CANCELED_ACTIVE' | 'EXPIRED' | 'REVOKED';

export type UsageMetricCode = 'AI_GENERATION' | 'QUOTE_EXPORT';
export type UsageResetPolicy = 'NEVER' | 'MONTHLY';

export type SubscriptionEventType =
  | 'PURCHASED' | 'RENEWED' | 'TRIAL_STARTED' | 'TRIAL_CONVERTED'
  | 'CANCELED' | 'EXPIRED' | 'GRACE_PERIOD_STARTED' | 'BILLING_RETRY_STARTED'
  | 'REVOKED' | 'RESTORED';
...
export type ActivityAction =
  | 'CREATED' | 'UPDATED' | 'STATUS_CHANGED' | 'IMAGE_UPLOADED'
  | 'GENERATION_STARTED' | 'GENERATION_COMPLETED' | 'ESTIMATE_CREATED'
  | 'ESTIMATE_UPDATED' | 'PROPOSAL_SENT' | 'PROPOSAL_VIEWED'
  | 'PROPOSAL_APPROVED' | 'PROPOSAL_DECLINED' | 'INVOICE_CREATED'
  | 'INVOICE_SENT' | 'INVOICE_PAID';

// Feature codes
export type FeatureCode =
  | 'CAN_GENERATE_PREVIEW' | 'CAN_EXPORT_QUOTE' | 'CAN_REMOVE_WATERMARK'
...
export type PaywallPlacement =
  | 'ONBOARDING_SOFT_GATE' | 'POST_FIRST_GENERATION' | 'POST_FIRST_QUOTE_EXPORT'
  | 'GENERATION_LIMIT_HIT' | 'QUOTE_LIMIT_HIT' | 'INVOICE_LOCKED'
  | 'BRANDING_LOCKED' | 'APPROVAL_SHARE_LOCKED' | 'WATERMARK_REMOVAL_LOCKED'
  | 'SETTINGS_UPGRADE';
