...
/**
 * Default visits-per-month for each recurrence cadence when the contractor
 * hasn't explicitly overridden it. The lawn-care prompt module reads this
 * via `PromptContext.visitsPerMonth` so per-visit consumables are quoted
 * against an honest visit count.
 */
function defaultVisitsPerMonth(
  ...
  switch (frequency) {
    ...
    case "quarterly":
      return 1 / 3;
    case "seasonal":
      // Seasonal cleanups: spring + fall + maybe a midseason → ~1/mo
      // averaged over the contract.
      return 1;
    default:
      return null;
  }
...
/**
 * Maps the Prisma `RecurrenceFrequency` text column onto the strict
 * `RecurrenceFrequency` union the prompt library expects. Returns null
 * when the value isn't recognized so the type stays narrow downstream.
 */
function mapRecurrenceFrequency(
  raw: string | null | undefined,
): "WEEKLY" | "BIWEEKLY" | "MONTHLY" | "QUARTERLY" | "SEASONAL" | null {
  switch (raw) {
    case "weekly":
      return "WEEKLY";
    case "biweekly":
...
/**
 * Build image generation context from the project data. Surfaces the
 * property measurements, recurrence terms, and (optional) company ZIP so
 * per-type prompt modules can ground their estimates against measured
 * area, real visit counts, and the contractor's local supply chain.
 */
function buildImageContext(
  project: {
    ...
    lawnAreaSqFt?: unknown;
    roofAreaSqFt?: unknown;
    isRecurring?: boolean;
    recurrenceFrequency?: string | null;
    visitsPerMonth?: unknown;
    contractMonths?: number | null;
  },
  companyZip?: string | null,
): ImageGenContext {
  const lawn = project.lawnAreaSqFt ? Number(project.lawnAreaSqFt) : null;
  const roof = project.roofAreaSqFt ? Number(project.roofAreaSqFt) : null;
  const explicitVisits = project.visitsPerMonth
    ? Number(project.visitsPerMonth)
    : null;
  const visits =
    explicitVisits ??
    (project.isRecurring
      ? defaultVisitsPerMonth(project.recurrenceFrequency ?? null)
      : null);
  ...
  return {
    ...
    lawnAreaSqFt: Number.isFinite(lawn) ? lawn : null,
    roofAreaSqFt: Number.isFinite(roof) ? roof : null,
    zip: companyZip ?? null,
    isRecurring: project.isRecurring ?? false,
    recurrenceFrequency: mapRecurrenceFrequency(
      project.recurrenceFrequency ?? null,
    ),
    visitsPerMonth: visits,
    contractMonths: project.contractMonths ?? null,
  };
}
...
async function processGeneration(
  ...
  try {
    ...
    if (!env.PIAPI_API_KEY && !env.GOOGLE_AI_API_KEY) {
      logger.warn(
        { generationId },
        "No image generation provider configured — marking generation as failed",
      );
      await prisma.aIGeneration.update({
        where: { id: generationId },
        data: {
    ...
    if (originalAsset?.imageData && originalAsset?.imageMimeType) {
      ...
      logger.info(
        { generationId, projectId, assetId: originalAsset.id },
        "Found reference photo for generation",
      );
    } else {
      logger.info(
        { generationId, projectId },
        "No reference photo found — generating without input image",
      );
    }
    ...
    generateAndStoreMaterials(
      ...
      logger.error(
        { err, generationId },
        "Material suggestion generation failed (non-critical)",
      );
    });
  } catch (err) {
...
async function generateAndStoreMaterials(
  ...
  const matContext: MaterialGenContext = {
    ...
    lawnAreaSqFt: context.lawnAreaSqFt,
    roofAreaSqFt: context.roofAreaSqFt,
    zip: context.zip,
    isRecurring: context.isRecurring,
    recurrenceFrequency: context.recurrenceFrequency,
    visitsPerMonth: context.visitsPerMonth,
    contractMonths: context.contractMonths,
  };
  ...
  logger.info(
    {
      ...
      laborCount: laborRecords.length,
    },
  ...
  try {
    ...
    logger.error(
      { err, generationId, projectId },
      "Auto-estimate creation failed (non-critical)",
    );
  }
}
 ...
 */
const DEFAULT_LABOR_BY_PROJECT_TYPE: Record<
  string,
...
> = {
  ...
  // LAWN_CARE is a recurring contract; the per-visit fallback assumes a
  // small commercial / HOA route stop. Real bids should always come from
...
async function autoCreateEstimate(
  ...
  if (existingEstimates.length > 0) {
    ...
    logger.info(
      ...
      "Cleared existing estimate line items for repopulation",
    );
  } else {
  ...
  if (!hasGeneratedLabor) {
    const labor =
      DEFAULT_LABOR_BY_PROJECT_TYPE[projectType] ??
      DEFAULT_LABOR_BY_PROJECT_TYPE.CUSTOM;
    lineItems.push({
      category: "labor" as const,
      name: `${projectType
        .replace(/_/g, " ")
  ...
  if (
    project &&
    ["DRAFT", "PHOTOS_UPLOADED", "GENERATING", "GENERATION_COMPLETE"].includes(
      project.status,
    )
  ) {
...
export async function create(
  ...
  // ZIP gives the prompt library localized supplier search hints; a missing
  // value just falls back to nationally-searchable queries — no hard
  // dependency.
  const company = await prisma.company.findUnique({
    where: { id: companyId },
    select: { zip: true },
  });
  const imageContext = buildImageContext(project, company?.zip ?? null);
