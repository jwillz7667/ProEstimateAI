...
export async function listHandler(req: Request, res: Response) {
  ...
  sendSuccess(
    res,
    result.items.map(toClientDto),
    { pagination: { next_cursor: result.nextCursor } }
  );
}

export async function getByIdHandler(req: Request, res: Response) {
  const client = await clientsService.getById(req.params.id as string, req.companyId!);
  sendSuccess(res, toClientDto(client));
}

export async function createHandler(req: Request, res: Response) {
  const client = await clientsService.create(req.companyId!, req.body);
  sendSuccess(res, toClientDto(client), { statusCode: 201 });
}

export async function updateHandler(req: Request, res: Response) {
  const client = await clientsService.update(req.params.id as string, req.companyId!, req.body);
  sendSuccess(res, toClientDto(client));
}

export async function deleteHandler(req: Request, res: Response) {
  await clientsService.remove(req.params.id as string, req.companyId!);
  sendSuccess(res, {});
}
