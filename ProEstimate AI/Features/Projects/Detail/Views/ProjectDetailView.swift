...
struct ProjectDetailView: View {
    ...
    /// When set (typically from a "preview ready" notification tap), the AI
    /// preview section opens focused on this generation's before/after
    /// comparison instead of the newest one.
    ...
    /// Get-paid loop sheet state. `presentedProposal` drives the
    /// proposal-preview sheet; `isCreatingProposal` surfaces the progress
    /// overlay while the backend mints the proposal. Invoice creation lives in
    /// the estimate→invoice conversion flow below.
    ...
    /// Estimate→invoice conversion state. `convertingEstimateId` drives the
    /// inline spinner on the estimate row; `convertedInvoice` triggers a sheet
    /// presenting the freshly-created invoice (presented modally rather than
    /// pushed to avoid a cross-tab navigation race onto the Invoices stack).
    @State private var convertingEstimateId: String?
    ...
    var body: some View {
        ...
        .sheet(item: $presentedProposal, onDismiss: {
            Task { await viewModel.reloadProposals() }
        }) { proposal in
            ProposalPreviewView(proposal: proposal)
        }
        .sheet(item: $convertedInvoice) { invoice in
            NavigationStack {
                InvoiceDetailView(invoiceId: invoice.id, isPresentedInSheet: true)
            }
        }
    ...
    private func projectContent(_ project: Project) -> some View {
        ScrollView {
            LazyVStack(spacing: SpacingTokens.lg) {
                ...
                ProjectOverviewSection(
                    ...
                    onClientTapped: {
                        if let clientId = project.clientId {
                            router.push(.clientDetail(id: clientId), on: appState.selectedTab)
                        }
                    }
                )
                ...
                // AI Preview — shown only when the project opts into image
                // generation. Service trades and projects the user switched to
                // text-only skip the before/after generator entirely; they
                // still get materials + an estimate below. We keep showing the
                // section if prior generations exist so a project switched off
                // after the fact doesn't hide its own history.
                if project.aiPreviewEnabled || !viewModel.generations.isEmpty {
                    AIPreviewSection(
                ...
                ProjectEstimatesSection(
                    ...
                    convertingEstimateId: convertingEstimateId,
                    ...
                    onConvertToInvoice: { estimateId in
                        handleConvertToInvoice(estimateId: estimateId)
                    }
                )

                // Proposals — the approval half of the get-paid loop. Invoices
                // live in their own top-level tab; estimates convert to invoices
                // via the row action above.
        ...
        .overlay {
            if isCreatingEstimate || isGeneratingAIEstimate || isCreatingProposal || exportProgressMessage != nil {
    ...
    // MARK: - Text-Only Estimate Note

    /// Replaces the AI preview generator for projects that opted out of image
    /// generation. Reassures the user the estimate path still works and points
    /// them at the edit sheet if they want to turn previews back on.
    private var textOnlyEstimateNote: some View {
        GlassCard {
            HStack(alignment: .top, spacing: SpacingTokens.sm) {
                ...
                VStack(alignment: .leading, spacing: SpacingTokens.xxs) {
                    Text("Text-only estimate")
                        .font(TypographyTokens.headline)
                    Text("This project skips the AI design preview. You'll still get itemized materials and labor below. To turn the before/after preview on, edit the project and enable “Generate AI design preview.”")
    ...
    /// Convert an estimate into a Pro-only invoice. Gated client-side through
    /// `guardCreateInvoice()` so free users hit the paywall before any network
    /// work. On success the new invoice is presented modally for immediate
    /// review. If the server still rejects with a 402 (entitlement drift), we
    /// re-present the paywall; other failures — most commonly "the project has
    /// no client" — surface via the standard error alert.
    private func handleConvertToInvoice(estimateId: String) {
        let result = featureGateCoordinator.guardCreateInvoice()
        switch result {
        case .allowed:
            guard convertingEstimateId == nil else { return }
            convertingEstimateId = estimateId
            Task {
                let service = LiveInvoiceService()
                do {
                    let invoice = try await service.convertEstimate(
                        estimateId: estimateId,
                        request: .faithful
                    )
                    await MainActor.run {
                        convertingEstimateId = nil
                        convertedInvoice = invoice
                    }
                } catch {
                    await MainActor.run {
                        convertingEstimateId = nil
                        if case let APIError.paywall(decision) = error {
                            paywallPresenter.present(decision)
                        } else {
                            viewModel.errorMessage = error.localizedDescription
                        }
